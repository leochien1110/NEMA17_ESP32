...
const int BUTTON_PIN =
    0; // Button pin connected to GPIO 0 for revolution adjustment

// Motor parameters
const int STEPS_PER_REV =
    1600; // TMC2209 Standalone Mode (1/8 microstepping: 200 * 8)
const int CLOCKWISE_DELAY =
    1000; // Microseconds between steps for clockwise (safe speed)
const int COUNTERCLOCKWISE_DELAY =
    2000; // Microseconds between steps for counterclockwise (2x slower)
...
bool clockwise = true;
int currentRevIndex =
    0; // Index into REV_OPTIONS array (starts with 1 revolution)
int currentRevolutions =
    REV_OPTIONS[currentRevIndex]; // Current revolution count
...
void setup() {
  ...
  Serial.begin(115200);
  Serial.println("TMC2209 NEMA17 Stepper Motor Controller");
